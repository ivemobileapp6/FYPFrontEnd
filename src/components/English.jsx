...
//     const isBadWord = filter.isProfane(word.toLowerCase());
//     if (isBadWord) {
...
import React, { useState, useEffect } from 'react';
import { Container, Row, Col, Form, Button, Table, Card } from 'react-bootstrap';
import { collection, addDoc, onSnapshot, query, orderBy, doc, deleteDoc, updateDoc, getDoc } from 'firebase/firestore';
...
const EnglishTutor = () => {
  ...
  const deleteWord = async (wordId) => {
    try {
      await deleteDoc(doc(wordsRef, wordId));
            ...
            <tbody>
              {words.map((entry, index) => (
                <tr key={index}>
                  ...
                  {userRole === 'admin' && (
                    <td>
                      <Button variant="danger" onClick={() => deleteWord(entry.id)}>
                        Delete
                      </Button>
                    </td>
                  )}
